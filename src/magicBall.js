...
function MagicBall() {
  ...
  function getRandomAnswer() {
    ...
    return setAnswer(arrayAnswers[index]);
  }
  ...
  return (
    <div className="magic-ball-container">
      <div className="question-container">
        <label className="question">Ваш вопрос: </label>
        <input className="css-input" onChange={handleQuestion} />
        <button className="testbutton" onClick={getRandomAnswer}>
          Спросить!
        </button>
      </div>
      <div className="ball-container">
        <p className="ball-question">{question}</p>
        <p className= "magic-ball">{answer}</p>
      </div>
    </div>
  );
}
